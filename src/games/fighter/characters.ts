...
function drawPersonaggio13(ctx: CanvasRenderingContext2D, x, y, w, h, style: any = {}) {
    ...
    // +BASTONE MAGICO (Dietro il corpo, lato destro - contenuto nella bounding box)
    const staffX = startX + w * 0.8;
    // Asta di legno
    ctx.beginPath();
    ctx.fillStyle = "#5c4033"; // Marrone scuro
    ctx.rect(staffX, startY + hatH * 0.5, w * 0.12, h * 0.85);
    ctx.fill();
    // Gemma magica (Esterna)
    ctx.beginPath();
    ctx.fillStyle = "#008b8b"; // Ciano scuro
    ctx.rect(staffX - w * 0.06, startY + hatH * 0.1, w * 0.24, hatH * 0.5);
    ctx.fill();
    // Gemma magica (Luce interna)
    ctx.beginPath();
    ctx.fillStyle = magicColor; 
    ctx.rect(staffX - w * 0.02, startY + hatH * 0.15, w * 0.16, hatH * 0.3);
    ctx.fill();
    // -bastone
    ...
    // Tesa del cappello (ridotta per stare dentro la bounding box)
    ctx.rect(startX + w * 0.05, startY + hatH * 0.8, w * 0.9, hatH * 0.2);
    ctx.fill();
    ...
    // +TUNICA E CORPO (Braccia dentro la bounding box)
    const bodyStartY = startY + hatH + headH;
    const sleeveW = w * 0.15; // Larghezza maniche ridotta
    const handW = w * 0.1;   // Larghezza mani
    
    // Maniche
    ctx.beginPath();
    ctx.fillStyle = robeColor;
    ctx.rect(startX - sleeveW * 0.8, bodyStartY, sleeveW * 0.8, bodyH * 0.7); // Manica sx
    ctx.rect(startX + w + sleeveW * 0.2, bodyStartY, sleeveW * 0.8, bodyH * 0.7); // Manica dx
    ctx.fill();

    // Mani che spuntano (centrate sotto la rispettiva manica)
    ctx.beginPath();
    ctx.fillStyle = skinColor;
    ctx.rect(startX - sleeveW * 0.4 - handW/2, bodyStartY + bodyH * 0.7, handW, bodyH * 0.15); // Mano sx
    ctx.rect(startX + w + sleeveW * 0.4, bodyStartY + bodyH * 0.7, handW, bodyH * 0.15); // Mano dx
    ctx.fill();
...
function drawNormalGuy(ctx: CanvasRenderingContext2D, x, y, w, h, style: any = {}) {
    ...
    ctx.rect(startX - armLen * 0.8, bodyStartY, armLen * 0.8, 0.35*bodyH); // left arm
    ctx.rect(startX + w + armLen * 0.2, bodyStartY, armLen * 0.8, 0.35*bodyH); // right arm
    ctx.fill();
    // -body
